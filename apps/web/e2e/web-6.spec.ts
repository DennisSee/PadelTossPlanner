...
test("staff beheert capaciteit en onafhankelijke sportprofielen zonder membership", async ({ page, request }) => {
  ...
  await page.getByRole("link", { name: /Leden en sportprofielen beheren/u }).click();
  await expect(page).toHaveURL(/\/beheer\/leden$/u);
  await page.getByLabel("Zoeken op naam").fill("Ready Partieel");
...
test("accountnaam blijft een smalle eigen mutation en wordt direct herladen", async ({ page, request }) => {
  ...
  await page.getByLabel("Zichtbare naam").fill(name);
  await page.getByRole("button", { name: "Naam opslaan" }).click();
  await expect(page).toHaveURL(/\/account\?notice=display-name-updated$/u);
  await expect(page.getByRole("status")).toContainText("naam is bijgewerkt");
  await expect(page.getByLabel("Zichtbare naam")).toHaveValue(name);
